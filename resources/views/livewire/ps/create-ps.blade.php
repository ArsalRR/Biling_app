<div>
  ...
  <form class="space-y-5" wire:submit.prevent="submit">
    <div>
      ...
      @error('nama_ps') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
    </div>
    <div>
      <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nomer Playstation</label>
      <input type="text" id="description" wire:model="nomor_ps" 
      ...
      @error('nomor_ps') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
    </div>
<div>
      <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">IP TV</label>
      <input type="text" id="description" wire:model="ip_tv" 
        class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white p-2.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"></textarea>
      @error('ip_tv') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
    </div>
    <!-- Harga -->
