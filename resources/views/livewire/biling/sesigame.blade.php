    ...
    @forelse($sesiGaming as $sesi)
      <div class="bg-white rounded-2xl shadow p-5 flex flex-col justify-between hover:shadow-lg transition">
        <div class="flex items-center justify-between mb-4">
          ...
          <span class="px-2 py-1 text-xs rounded-lg font-medium bg-{{ $sesi->getStatusBadgeColor() }}-100 text-{{ $sesi->getStatusBadgeColor() }}-700">
            {{ ucfirst($sesi->status) }}
          </span>
        </div>
        ...
        <div class="mt-4 flex gap-2 flex-wrap">
          @if($sesi->status === 'aktif')
            <button wire:click="openExtendModal({{ $sesi->id }})"
                    class="flex-1 min-w-20 bg-purple-500 text-white p-2 rounded-lg hover:bg-purple-600 flex items-center justify-center gap-1">
            ...
            <button wire:click="selesaikanSesi({{ $sesi->id }})"
                    ...
                    class="flex-1 min-w-20 bg-green-500 text-white p-2 rounded-lg hover:bg-green-600 flex items-center justify-center gap-1">
            ...
            <button wire:click="batalkanSesi({{ $sesi->id }})"
                    ...
                    class="flex-1 min-w-20 bg-yellow-400 text-white p-2 rounded-lg hover:bg-yellow-500 flex items-center justify-center gap-1">
          ...
          <button wire:click="hapusSesi({{ $sesi->id }})"
                  ...
                  class="flex-1 min-w-20 bg-red-500 text-white p-2 rounded-lg hover:bg-red-600 flex items-center justify-center gap-1">
      ...
      <form wire:submit.prevent="save">
        <div class="p-6 space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium mb-1">Paket Playstation</label>
              <select wire:model="paket_id" class="w-full border rounded-xl px-3 py-2">
                <option value="">Pilih Paket</option>
                @foreach($pakets as $paket)
                  <option value="{{ $paket->id }}">{{ $paket->nama }} - Rp {{ number_format($paket->harga, 0, ',', '.') }} ({{ $paket->durasi_menit }}m)</option>
                @endforeach
              </select>
              @error('paket_id') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
            </div>
            <div>
              <label class="block text-sm font-medium mb-1">Tempat Playstation</label>
              <select wire:model="ps_id" class="w-full border rounded-xl px-3 py-2">
                <option value="">Pilih Tempat</option>
                @foreach($ps as $p)
                  <option value="{{ $p->id }}">{{ $p->nomor_ps }} - {{ $p->nama_ps }}</option>
                @endforeach
              </select>
              @error('ps_id') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="datetime-local" wire:model="waktu_mulai" class="w-full border rounded-xl px-3 py-2">
              @error('waktu_mulai') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
            </div>
          </div>
        ...
        <div class="flex justify-end gap-2 border-t px-6 py-4">
          <button type="button" wire:click="closeModal" class="px-4 py-2 rounded-xl bg-gray-100 hover:bg-gray-200">Batal</button>
    ...
    <div class="bg-white rounded-2xl shadow-lg w-full max-w-lg">
      <div class="p-6">
        <div class="flex justify-between items-center mb-4">
          ...
          <button wire:click="closeExtendModal" class="text-gray-400 hover:text-gray-600">
            <i class="fas fa-times"></i>
          </button>
        </div>
        ...
        <form wire:submit.prevent="extendTime">
          <div class="space-y-4">
            ...
            <div>
              <label class="block text-sm font-medium mb-2">Pilih Paket Perpanjangan</label>
              <select wire:model="extendPaketId" class="w-full border rounded-xl px-3 py-2">
                <option value="">Pilih Paket</option>
                @foreach($pakets as $paket)
            ...
            @if($extendPaketId && $tambahanWaktu > 0)
              <div class="bg-blue-50 p-4 rounded-lg">
                <div class="text-sm">
                  @php
                    $selectedPaket = $pakets->where('id', $extendPaketId)->first();
                    $currentSesi = $extendSesiId ? $sesiGaming->where('id', $extendSesiId)->first() : null;
                  @endphp
                  @if($selectedPaket && $currentSesi)
                    <div class="flex justify-between">
                      <span class="text-gray-600">Tambahan Waktu:</span>
         ...
         <button type="submit"
        class="px-4 py-2 rounded-xl bg-purple-600 text-white hover:bg-purple-700 disabled:opacity-50 flex items-center"
        wire:loading.attr="disabled"
        wire:target="extendTime"
        @disabled(!$extendPaketId)>
  <i class="fas fa-clock mr-2"></i>
  <span wire:loading.remove wire:target="extendTime">Perpanjang Waktu</span>
  <span wire:loading wire:target="extendTime">Memproses...</span>
</button>
...
<script>
  // Auto refresh setiap 30 detik untuk update sisa waktu
  ...
  window.addEventListener('alert', function(e) {
    ...
    alertDiv.className = `flex items-center max-w-sm p-4 border rounded-lg shadow fixed top-4 right-4 z-50`;
    
    if (alertType === 'success') {
      alertDiv.className += ' text-green-700 bg-green-100 border-green-300';
      alertDiv.innerHTML = `
        <svg class="w-5 h-5 mr-2 shrink-0" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414L9 14.414 5.293 10.707a1 1 0 011.414-1.414L9 11.586l6.293-6.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
        </svg>
      ...
      alertDiv.className += ' text-red-700 bg-red-100 border-red-300';
      alertDiv.innerHTML = `
        <svg class="w-5 h-5 mr-2 shrink-0" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm-1-4h2v2H9v-2zm0-8h2v6H9V6z"/>
        </svg>
      ...
      alertDiv.className += ' text-yellow-700 bg-yellow-100 border-yellow-300';
      alertDiv.innerHTML = `
        <svg class="w-5 h-5 mr-2 shrink-0" fill="currentColor" viewBox="0 0 20 20">
